...
class InstanceToOneHot_rgb(object):
    """
        Pre-processor that changes instances into index based labels
    ...
    def pre_process(self, data, flag=False):
        new_data = np.zeros(data.shape)
        for i in range(self.nbr_classes):
            r, g, b = self.class_order[i+1]
            mask_i = (data == [r, g, b]).all(-1)
            new_data[mask_i] = i+1
            # new_data[mask_i] = i+1
            flag = False
        if (new_data[:,:, 0] == new_data[:,:, 1]).all() and (new_data[:,:, 2] == new_data[:,:, 1]).all():
            # print("Same value in all channels!")
            new_data = new_data[:,:,0]
        #print("old data" + str(data.shape))
        #print("new data" + str(new_data.shape))
        # print(new_data)
...
def check_for_nan_inf(x):
    if np.isnan(np.min(x)):
        raise ValueError('There exist NaN pixels in the images!!!')

    if np.isinf(np.min(x)) > 0:
        raise ValueError('There exist Inf pixels in the images!!!')
