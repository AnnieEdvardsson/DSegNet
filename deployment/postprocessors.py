        ...
        for i in range(self.x_dim):
            for j in range(self.y_dim):
                ...
                if one_hot_softmax[i, j, m] > self.prob_threshold:
                    instances_grayscale[i, j] = m #self.class_list[m]
...
class ColorConverter(object):
    """
        Convert all predicted images from grays cale to color
        :param label_to_color: The color mapping between a gray scale value and RGB-code
    """
    def __init__(self, label_to_color: dict):
        self.label_to_color = label_to_color

    def convert_all_images(self, images):

        batch, img_height, img_width = images.shape

        rgb_images = np.zeros(shape=(batch, img_height, img_width, 3))

        for i, img in enumerate(images):
            rgb_images[i] = self.gray2color(img)
    ...
    def gray2color(self, img):
        ...
        rgb_img = np.zeros((img_height, img_width, 3))

        for key in self.label_to_color.keys():
            rgb_img[img == key] = self.label_to_color[key]
