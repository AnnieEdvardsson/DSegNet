    ...
    def build_summaries(self):
        ...
        with tf.device('/cpu:0'):
            for i in range(self.num_scale):
                tf.summary.scalar('ssim_loss_' + str(i + 1), self.ssim_loss_left[i] + self.ssim_loss_right[i],
                                  collections=self.model_collection)
                tf.summary.scalar('l1_loss_' + str(i + 1),
                                  self.l1_reconstruction_loss_left[i] + self.l1_reconstruction_loss_right[i],
                                  collections=self.model_collection)
                tf.summary.scalar('image_loss_' + str(i + 1), self.image_loss_left[i] + self.image_loss_right[i],
                                  collections=self.model_collection)
                tf.summary.scalar('disp_gradient_loss_' + str(i + 1), self.disp_left_loss[i] + self.disp_right_loss[i],
                                  collections=self.model_collection)
                tf.summary.scalar('lr_loss_' + str(i + 1), self.lr_left_loss[i] + self.lr_right_loss[i],
                                  collections=self.model_collection)
                tf.summary.image('disp_left_est_' + str(i + 1), self.disp_left_est[i], max_outputs=1,
                                 collections=self.model_collection)
                tf.summary.image('disp_right_est_' + str(i + 1), self.disp_right_est[i], max_outputs=1,
                                 collections=self.model_collection)

                if self.params.full_summary:
                    tf.summary.image('left_est_' + str(i + 1), self.left_est[i], max_outputs=1,
                                     collections=self.model_collection)
                    tf.summary.image('right_est_' + str(i + 1), self.right_est[i], max_outputs=1,
                                     collections=self.model_collection)
                    tf.summary.image('ssim_left_' + str(i + 1), self.ssim_left[i], max_outputs=1,
                                     collections=self.model_collection)
                    tf.summary.image('ssim_right_' + str(i + 1), self.ssim_right[i], max_outputs=1,
                                     collections=self.model_collection)
                    tf.summary.image('l1_left_' + str(i + 1), self.l1_left[i], max_outputs=1,
                                     collections=self.model_collection)
                    tf.summary.image('l1_right_' + str(i + 1), self.l1_right[i], max_outputs=1,
                                     collections=self.model_collection)
